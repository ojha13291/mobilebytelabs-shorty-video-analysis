...
- **Short-Form Video Analysis**: Specialized in analyzing Instagram Reels and short-form video content
- **AI-Powered Insights**: Advanced sentiment analysis, content categorization, and engagement prediction using Mistral AI
- **Performance Metrics**: Comprehensive analytics including views, likes, comments, and engagement rates
- **Content Understanding**: Automatic video content summarization and keyword extraction
- **Creator Analytics**: Detailed creator profile analysis and audience insights
- **Scalable Architecture**: Modular microservice design with Docker support for easy deployment
- **Real-time Processing**: Fast analysis with comprehensive error handling and health monitoring

## 🏗️ Architecture Overview

Shorty Video Analysis follows a modular microservice architecture designed for scalability and maintainability:

```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   API Gateway   │───▶│  Analysis Core  │───▶│   AI Services   │
│   (Flask API)   │    │  (Processing)   │    │ (Mistral AI)    │
└─────────────────┘    └─────────────────┘    └─────────────────┘
         │                       │                       │
         ▼                       ▼                       ▼
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│  Data Storage   │◀───│ Content Scraper │◀───│  External APIs  │
│   (Optional)    │    │  (Instagram)    │    │  (Instagram)    │
└─────────────────┘    └─────────────────┘    └─────────────────┘
```

### Key Components

- **API Gateway**: RESTful endpoints for video analysis requests
- **Content Scraper**: Instagram Reels data extraction using Instaloader/Selenium
- **Analysis Core**: Video content processing and feature extraction
- **AI Services**: Mistral AI integration for sentiment and content analysis
- **Health Monitor**: Service health checks and metrics collection
...
GET /health
```
Returns service health status and component availability for the Shorty Video Analysis service.
...
POST /api/analyze
```
Main endpoint for analyzing Instagram Reels with AI-powered insights.

**Request Body:**
```json
{
  "target": "@username",           // Instagram username, hashtag, or URL
  "max_reels": 10,                // Maximum reels to analyze (1-50, default: 10)
  "use_login": true,              // Use Instagram login for enhanced access
  "scraping_method": "instaloader", // "instaloader" (recommended) or "selenium"
  "include_comments": true,       // Include comment analysis (default: true)
